...
class EBALOHA(BaseAgent):
    ...
    def update(state, key, env_state, action, reward, terminal, window_size: int, max_backoff: int):
        _, channel = select_features(env_state, EBALOHA.FEATURES)[-1]
        transmitted = action == Actions.TX.value
        success = transmitted & (channel == 1)
        collision = transmitted & (channel != 1)
    ...
    def sample(state, key, env_state):
        buffer, _ = select_features(env_state, EBALOHA.FEATURES)[-1]
